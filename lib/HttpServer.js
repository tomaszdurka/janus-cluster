...
function HttpServer(port, cluster) {
  ...
  router.post('/unregister', this.unregister.bind(this));
...
HttpServer.prototype.register = function(req, res, next) {
  ...
  this.cluster.registerMember(member)
    ...
    .catch(function(error) {
      logContext.extend({exception: error});
      log.error('HTTP: Could not register member', logContext);
      res.send({error: 'Could not register member: ' + error.message});
      return next();
    });
};

HttpServer.prototype.unregister = function(req, res, next) {
  var params = _.extend({}, req.params, req.body);
  var logContext = new Context({httpRequest: params});
  log.debug('HTTP: Unregistering member', logContext);

  var member = this.cluster.findMemberById(params['id']);
  if (!member) {
    log.debug('Member not found', logContext);
    res.send({success: 'Member not found'});
    return next();
  }
  logContext.extend({member: member});
  
  this.cluster.unregisterMember(member)
    .then(function(response) {
      res.send({success: 'Member unregistered'});
      return next();
    })
    .catch(function(error) {
      logContext.extend({exception: error});
      log.error('HTTP: Could not unregister member', logContext);
      res.send({error: 'Could not unregister member: ' + error.message});
      return next();
    });
};
...
HttpServer.prototype.status = function(req, res, next) {
  ...
  res.send(JSON.stringify(members, null, 4));
  return next();
};
